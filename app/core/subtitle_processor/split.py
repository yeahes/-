...
def preprocess_segments(
    ...
    for seg in segments:
        if not is_pure_punctuation(seg.text):
            ...
            if re.search(r"[a-zA-Z0-9]", seg.text.strip()) and seg.text.strip().isascii():
                if need_lower:
                    seg.text = seg.text.strip().lower() + " "
                else:
...
class SubtitleSplitter:
    ...
    def split_subtitle(self, subtitle_data: Union[str, ASRData]) -> ASRData:
        ...
        try:
            ...
            if self.screen_mode:
                final_segments = self._enforce_screen_length(final_segments)
    ...
    def merge_short_segment(self, segments: List[ASRDataSeg]) -> None:
        ...
        while i < len(segments) - 1:  # 修改遍历方式
            ...
            short_word_limit = 3 if self.screen_mode else 5
            tiny_word_limit = 2 if self.screen_mode else 3
            tight_gap = 160 if self.screen_mode else 200
            loose_gap = 320 if self.screen_mode else 500

            if (
                time_gap < tight_gap
                and (current_words < short_word_limit or next_words < short_word_limit)
                and total_words <= max_word_count
            ) or (
                time_gap < loose_gap
                and (current_words < tiny_word_limit or next_words < tiny_word_limit)
                and total_words <= max_word_count
            ):
                # 执行合并操作
                logger.debug(
    ...
    def _enforce_screen_length(self, segments: List[ASRDataSeg]) -> List[ASRDataSeg]:
        """Keep English screen subtitles under the configured word limit."""
        result: List[ASRDataSeg] = []
        for seg in segments:
            if is_mainly_cjk(seg.text) or count_words(seg.text) <= self.max_word_count_english:
                result.append(seg)
                continue
            result.extend(self._split_english_segment_for_screen(seg))
        return result

    def _split_english_segment_for_screen(self, seg: ASRDataSeg) -> List[ASRDataSeg]:
        parts = self._split_english_text_for_screen(seg.text, self.max_word_count_english)
        if len(parts) <= 1:
            return [seg]
        ...
        for index, part in enumerate(parts):
            ...
            result.append(
                ASRDataSeg(
                    text=part,
                    translated_text=seg.translated_text if index == 0 else "",
    ...
    @classmethod
    def _split_english_text_for_screen(cls, text: str, max_words: int) -> List[str]:
        text = " ".join(text.split())
        if count_words(text) <= max_words:
            return [text]

        clauses = re.split(
            r"(?<=[,;:!?])\s+|(?=\b(?:because|but|so|which|when|where|while|and|or|that|who)\b)",
            text,
            flags=re.IGNORECASE,
        )
        parts: List[str] = []
        current = ""
        for clause in [piece.strip() for piece in clauses if piece.strip()]:
            candidate = f"{current} {clause}".strip() if current else clause
            if current and count_words(candidate) > max_words:
                parts.extend(cls._split_by_word_count(current, max_words))
                current = clause
            else:
                current = candidate
        if current:
