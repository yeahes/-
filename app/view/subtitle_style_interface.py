...
from app.core.utils.rounded_subtitle import RoundedBgStyle, render_preview as render_rounded_preview
...
class PreviewThread(QThread):
    ...
    def __init__(
        ...
        self.rounded_style = rounded_style or {}

    def run(self):
        if self.render_mode == "圆角背景":
            preview_path = render_rounded_preview(
                primary_text=self.preview_text[0],
                secondary_text=self.preview_text[1] or "",
                bg_image_path=self.bg_path,
                width=self.width,
                height=self.height,
                style=RoundedBgStyle(**self.rounded_style),
            )
        else:
...
class SubtitleStyleInterface(QWidget):
    ...
    def _initSettingsArea(self):
        ...
        self.roundedGroup = SettingCardGroup(self.tr("圆角背景样式"), self.settingsWidget)
    ...
    def _initSettingCards(self):
        ...
        self.renderModeCard = ComboBoxSettingCard(
            FIF.VIEW,
            self.tr("渲染模式"),
            self.tr("选择普通ASS字幕或圆角背景硬字幕"),
            texts=["ASS样式", "圆角背景"],
            parent=self.roundedGroup,
        )

        self.roundedFontCard = ComboBoxSettingCard(
            FIF.FONT,
            self.tr("圆角英文字体"),
            self.tr("英文或非中文圆角字幕使用的字体"),
            texts=["Arial"],
            parent=self.roundedGroup,
        )
        ...
        self.roundedTextColorCard = ColorSettingCard(
            ...
            FIF.PALETTE,
            self.tr("圆角英文颜色"),
            self.tr("英文或非中文圆角字幕文字颜色"),
            parent=self.roundedGroup,
        )

        self.roundedCjkFontCard = ComboBoxSettingCard(
            FIF.FONT,
            self.tr("圆角中文字体"),
            self.tr("中文圆角字幕使用的字体"),
            texts=["Microsoft YaHei"],
            parent=self.roundedGroup,
        )

        self.roundedCjkFontSizeCard = SpinBoxSettingCard(
            FIF.FONT_SIZE,
            self.tr("圆角中文字号"),
            self.tr("中文圆角字幕使用的字号"),
            minimum=8,
        ...
        self.roundedCjkTextColorCard = ColorSettingCard(
            QColor(255, 255, 255),
            FIF.PALETTE,
            self.tr("圆角中文颜色"),
            self.tr("中文圆角字幕文字颜色"),
        ...
        self.roundedBgColorCard = ColorSettingCard(
            ...
            FIF.PALETTE,
            self.tr("圆角背景颜色"),
            self.tr("圆角字幕背景颜色，可调整透明度"),
            parent=self.roundedGroup,
            enableAlpha=True,
        )

        self.roundedRadiusCard = SpinBoxSettingCard(
            FIF.PALETTE,
            self.tr("背景圆角"),
            self.tr("圆角背景半径"),
            minimum=0,
    ...
    def _initLayout(self):
        ...
        self.roundedGroup.addSettingCard(self.renderModeCard)
        self.roundedGroup.addSettingCard(self.roundedFontCard)
        self.roundedGroup.addSettingCard(self.roundedFontSizeCard)
        self.roundedGroup.addSettingCard(self.roundedTextColorCard)
        self.roundedGroup.addSettingCard(self.roundedCjkFontCard)
        self.roundedGroup.addSettingCard(self.roundedCjkFontSizeCard)
        self.roundedGroup.addSettingCard(self.roundedCjkTextColorCard)
        self.roundedGroup.addSettingCard(self.roundedBgColorCard)
        self.roundedGroup.addSettingCard(self.roundedRadiusCard)
        self.roundedGroup.addSettingCard(self.roundedPaddingHCard)
        self.roundedGroup.addSettingCard(self.roundedPaddingVCard)
        self.roundedGroup.addSettingCard(self.roundedMarginBottomCard)
        self.roundedGroup.addSettingCard(self.roundedLineSpacingCard)
        self.roundedGroup.addSettingCard(self.roundedLetterSpacingCard)
        ...
        self.settingsLayout.addWidget(self.roundedGroup)
    ...
    @staticmethod
    def _qcolor_from_hex(value: str) -> QColor:
    ...
    @staticmethod
    def _qcolor_to_hex(color: QColor, alpha: bool = False) -> str:
        if alpha:
    ...
    def getRoundedStyle(self) -> dict:
        return {
            "font_name": self.roundedFontCard.comboBox.currentText(),
            "font_size": self.roundedFontSizeCard.spinBox.value(),
            "text_color": self._qcolor_to_hex(self.roundedTextColorCard.colorPicker.color),
            "cjk_font_name": self.roundedCjkFontCard.comboBox.currentText(),
            "cjk_font_size": self.roundedCjkFontSizeCard.spinBox.value(),
            "cjk_text_color": self._qcolor_to_hex(
                self.roundedCjkTextColorCard.colorPicker.color
            ),
            "bg_color": self._qcolor_to_hex(self.roundedBgColorCard.colorPicker.color, alpha=True),
            "corner_radius": self.roundedRadiusCard.spinBox.value(),
    ...
    def saveRoundedConfig(self):
        cfg.set(cfg.subtitle_render_mode, self.renderModeCard.comboBox.currentText())
        cfg.set(cfg.rounded_font_name, self.roundedFontCard.comboBox.currentText())
        cfg.set(cfg.rounded_font_size, self.roundedFontSizeCard.spinBox.value())
        cfg.set(
            cfg.rounded_text_color,
            self._qcolor_to_hex(self.roundedTextColorCard.colorPicker.color),
        )
        cfg.set(
            cfg.rounded_cjk_font_name,
            self.roundedCjkFontCard.comboBox.currentText(),
        )
        cfg.set(cfg.rounded_cjk_font_size, self.roundedCjkFontSizeCard.spinBox.value())
        cfg.set(
            cfg.rounded_cjk_text_color,
            self._qcolor_to_hex(self.roundedCjkTextColorCard.colorPicker.color),
        )
        cfg.set(
            cfg.rounded_bg_color,
            self._qcolor_to_hex(self.roundedBgColorCard.colorPicker.color, alpha=True),
        )
        cfg.set(cfg.rounded_corner_radius, self.roundedRadiusCard.spinBox.value())
        cfg.set(cfg.rounded_padding_h, self.roundedPaddingHCard.spinBox.value())
        cfg.set(cfg.rounded_padding_v, self.roundedPaddingVCard.spinBox.value())
        cfg.set(cfg.rounded_margin_bottom, self.roundedMarginBottomCard.spinBox.value())
        cfg.set(cfg.rounded_line_spacing, self.roundedLineSpacingCard.spinBox.value())
        cfg.set(cfg.rounded_letter_spacing, self.roundedLetterSpacingCard.spinBox.value())
    ...
    def __setValues(self):
        ...
        self.renderModeCard.comboBox.setCurrentText(cfg.get(cfg.subtitle_render_mode))
        self.roundedFontSizeCard.spinBox.setValue(cfg.get(cfg.rounded_font_size))
        self.roundedTextColorCard.setColor(
            self._qcolor_from_hex(cfg.get(cfg.rounded_text_color))
        )
        self.roundedCjkFontSizeCard.spinBox.setValue(
            cfg.get(cfg.rounded_cjk_font_size)
        )
        self.roundedCjkTextColorCard.setColor(
            self._qcolor_from_hex(cfg.get(cfg.rounded_cjk_text_color))
        )
        self.roundedBgColorCard.setColor(
            self._qcolor_from_hex(cfg.get(cfg.rounded_bg_color))
        )
        self.roundedRadiusCard.spinBox.setValue(cfg.get(cfg.rounded_corner_radius))
        self.roundedPaddingHCard.spinBox.setValue(cfg.get(cfg.rounded_padding_h))
        self.roundedPaddingVCard.spinBox.setValue(cfg.get(cfg.rounded_padding_v))
        self.roundedMarginBottomCard.spinBox.setValue(cfg.get(cfg.rounded_margin_bottom))
        self.roundedLineSpacingCard.spinBox.setValue(cfg.get(cfg.rounded_line_spacing))
        self.roundedLetterSpacingCard.spinBox.setValue(cfg.get(cfg.rounded_letter_spacing))
        ...
        self.roundedFontCard.addItems(fontFamilies)
        self.roundedCjkFontCard.addItems(fontFamilies)
        ...
        self.roundedCjkFontCard.comboBox.setMaxVisibleItems(12)

        rounded_font_name = cfg.get(cfg.rounded_font_name)
        if rounded_font_name:
            self.roundedFontCard.setCurrentText(rounded_font_name)
        rounded_cjk_font_name = cfg.get(cfg.rounded_cjk_font_name)
        if rounded_cjk_font_name:
            self.roundedCjkFontCard.setCurrentText(rounded_cjk_font_name)
    ...
    def connectSignals(self):
        ...
        self.renderModeCard.currentTextChanged.connect(self.onSettingChanged)
        self.roundedFontCard.currentTextChanged.connect(self.onSettingChanged)
        self.roundedFontSizeCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedTextColorCard.colorChanged.connect(self.onSettingChanged)
        self.roundedCjkFontCard.currentTextChanged.connect(self.onSettingChanged)
        self.roundedCjkFontSizeCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedCjkTextColorCard.colorChanged.connect(self.onSettingChanged)
        self.roundedBgColorCard.colorChanged.connect(self.onSettingChanged)
        self.roundedRadiusCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedPaddingHCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedPaddingVCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedMarginBottomCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedLineSpacingCard.spinBox.valueChanged.connect(self.onSettingChanged)
        self.roundedLetterSpacingCard.spinBox.valueChanged.connect(self.onSettingChanged)
    ...
    def updatePreview(self):
        ...
        self.preview_thread = PreviewThread(
            ...
            width=width,
            height=height,
            render_mode=self.renderModeCard.comboBox.currentText(),
            rounded_style=self.getRoundedStyle(),
        )
